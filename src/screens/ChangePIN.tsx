...
export default function ChangePIN() {
  ...
  const [visibleConfirmChangePin, setVisibleConfirmChangePin] = useState(false);
  ...
  async function changePin() {
    ...
    if (currentPin == pinContext.pin) {
      if (!newPin || newPin?.length != 4 || !newPin?.match(/^[0-9]+$/)) {
        setAlertMessage(
          "New PIN is invalid! Please, enter 4 numeric characters."
        );
  ...
  return (
    <>
      ...
      <View style={styles.container}>
        ...
        <PrimaryButton
          text="CHANGE PIN"
          onPress={() => setVisibleConfirmChangePin(true)}
        />
      </View>
    </>
  );
